...
.output 最近三月振幅25到30个点以上.rst
.headers on
select tdate,symbol,close,sma10,round(sma20,2),round(sma30,2),round(sma60,2),round(sma250,2)--,sma250,max60,min60,abs((max60-min60)/min60*100)
...
.output 最近一年振幅超50个点以上.rst
.headers on
select tdate,symbol,close,abs((max250-min250)/min250*100) fudu--,sma250,max60,min60,abs((max60-min60)/min60*100)
from stockmd  where 1=1
and close > sma250
and  abs((max250-min250)/min250*100) > 50   --领先于大盘
order by fudu,close;

.output 最近三个月窄幅震荡.rst
.headers on
select tdate,symbol,close,sma10,round(sma20,2),round(sma30,2),round(sma60,2),round(sma250,2)--,sma250,max60,min60,abs((max60-min60)/min60*100)
from stockmd  where 1=1
and close > sma250
...
.output 相对三个月高点下跌超25%到35%之间.rst
.headers on
select tdate,symbol,close,sma10,round(sma20,2),round(sma30,2),round(sma60,2),round(sma250,2)--,sma250,max60,min60,abs((max60-min60)/min60*100)
from stockmd  where 1=1
and close > sma250
...
--十日均线附近
.output 年线以上-10日均线线附近0~5.rst
.headers on
select tdate,symbol,close,sma10,sma20--,sma250,max60,min60,abs((max60-min60)/min60*100)
from stockmd  where 1=1
and close > sma250
--and  abs((close-sma250)/sma250*100) between 0 and 5
...
--(300702三聚环保)
.output 年线以上-20日均线附近0~5-20日离250日均线25~30个点左右.rst
.headers on
select tdate,symbol,close,sma10,round(sma20,2),round(sma30,2),round(sma60,2),round(sma250,2)--,sma250,max60,min60,abs((max60-min60)/min60*100)
from stockmd  where 1=1
and close > sma250
...
--20日附近，且20日在250之上25~30个点左右，即将进入最后的疯狂
--(300702三聚环保)
.output 年线以上-20日均线附近0~5-20日离250日均线15~25个点左右.rst
.headers on
select tdate,symbol,close,sma10,round(sma20,2),round(sma30,2),round(sma60,2),round(sma250,2)--,sma250,max60,min60,abs((max60-min60)/min60*100)
from stockmd  where 1=1
and close > sma250
...
--20日附近，且20日在250之上25~30个点左右，即将进入最后的疯狂
--(300702三聚环保)
.output 年线以上-20日均线附近0~5-20日离250日均线5~15个点左右.rst
.headers on
select tdate,symbol,close,sma10,round(sma20,2),round(sma30,2),round(sma60,2),round(sma250,2)--,sma250,max60,min60,abs((max60-min60)/min60*100)
from stockmd  where 1=1
and close > sma250
...
--运行到30日线附近
--运行到60日线附近
.output 均线附近-年线以上-10日均线附近0~1.rst
.headers on
select tdate,symbol,close,sma10,round(sma20,2),round(sma30,2),round(sma60,2),round(sma250,2)--,sma250,max60,min60,abs((max60-min60)/min60*100)
from stockmd  where 1=1
and close > sma250
...
.output 均线附近-年线以上-20日均线附近-0.5~1.rst
.headers on
select tdate,symbol,close,sma10,round(sma20,2),round(sma30,2),round(sma60,2),round(sma250,2)--,sma250,max60,min60,abs((max60-min60)/min60*100)
from stockmd  where 1=1
and close > sma250
...
.output 均线附近-年线以上-30日均线附近0~1.rst
.headers on
select tdate,symbol,close,round(sma30,2)
from stockmd  where 1=1
and close > sma250
and  (close-sma30)/sma30*100 between 0 and 1
order by close;

.output 均线附近-年线以上-60日均线附近0~1.rst
.headers on
select tdate,symbol,close,round(sma60,2)
from stockmd  where 1=1
and close > sma250
...
.output 均线附近-年线以上-5%~15%领先大盘-60日均线附近0~1.rst
.headers on
select tdate,symbol,close,round(sma60,2)
from stockmd  where 1=1
and close > sma250
...
.output 弱支撑.rst
.headers on
select tdate,symbol,close,round(( max60 - close)/close*100 ,2)
from stockmd  where 1=1
and close > sma250
